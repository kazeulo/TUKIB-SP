...
In its current state, LIRA utilizes a set of predefined intents and responses based on a small, curated training dataset relevant to the Regional Research Center (RRC). During system testing, the chatbot demonstrated efficient response times and performed well when interacting with common queries. By responding instantly, it contributes to user engagement and minimizes dependency on human support.

\subsubsection*{Limitations}

While functional, the chatbot currently has several limitations. First, its training data is limited, which restricts the range and depth of questions it can effectively handle. As a result, the bot may fail to provide helpful responses to queries outside its training scope.
